...
@Component("analyzeBhava10")
public class AnalyzeBhava10 implements AnalyzeBhava{

	@Autowired
	@Qualifier("karmaBhavaHelper")
	HouseLordPlacedInBhava houseLordPlacedInBhava;
	
	@Autowired
	@Qualifier("grahaInBhava10")
	...
	public String analyzeBhava(Chart horoscope) {
		...
		String infoAboutGrahasInHouse = GrahaUtil.infoAboutGrahasInHouse(grahaInBhava,horoscope.getBhavas()[9]);
		String infoAboutRashiInHouse = RashiUtil.infoAboutRashiInHouse(rashiInBhava,horoscope.getBhavas()[9]);
		return infoAboutHouseLord + "\n" + infoAboutGrahasInHouse + "\n" + infoAboutRashiInHouse;

	}
	
	private String infoAboutHouseLord(Chart horoscope) {
		 return LordOfHouseUtil.infoOnLordofTenthHouse(houseLordPlacedInBhava,horoscope);
	}
